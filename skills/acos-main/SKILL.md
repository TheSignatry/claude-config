...
`acos-main` needs no profile fields of its own — it only orchestrates, and each sibling already reads what it needs from `acos-aboutme`'s `state/profile.json` (path `../acos-aboutme/state/profile.json` from here) via its own `plan` command. If a sibling's `plan` step fails because the profile is missing or thin, relay that sibling's own error message to Trevor and point him at `acos-aboutme` or the specific sibling to enroll — don't guess at the missing config to work around it.
...
4. Run `scripts/main_plan.py compose --perspective week-plan --calendar <report.json> --jira <report.json> --email-current <messages.json>`. Returns `category_bars` (one entry per benchmark category with its color, hours, %, and benchmark status — ready to render directly), `key_meetings`, `product_deliveries`, `support_tickets`, `critical_emails`.
5. **Write the HTML artifact**: a seven-row segmented horizontal bar, Monday through Sunday, each day's bar length representing working hours and its segments colored per `category_bars`' mapping, with a small legend; a benchmark-comparison prose paragraph (which categories ran over/under target this week, in Claude's own words, not a template fill-in); then four numbered lists in the same visual language as morning-plan — key meetings, critical emails, the 2-week product-delivery window, and support tickets needing attention.
...
5. **Write the HTML artifact**: four stacked sections in the same band/hairline/list language as the other two perspectives — (1) category-vs-benchmark-vs-prior-month mini bars (reuse week-plan's segmented-bar technique at a smaller scale) plus a Claude-authored "be more intentional about" paragraph grounded in the actual deltas, not a generic template; (2) delivered products; (3) support-ticket performance; (4) email-volume summary, noting `thin_data` plainly if set.

## Output format

Default: one self-contained HTML artifact via the Artifact tool — brand-core fonts (Lora, Mulish) embedded as base64 `@font-face`, mirrored locally from `../signatry-brand-core/assets/fonts/`, no external requests. Markdown only if Trevor explicitly asks for it — reuse each sibling's own `--format markdown` output where applicable, plus a plain textual rendering of `compose`'s output for the parts that don't come from a sibling directly (the highlight sets, category bars as a table instead of a chart). No chart in the markdown path — table and prose only.
