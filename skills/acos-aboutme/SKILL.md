---
name: acos-aboutme
description: "Shared identity and org-chart profile for the acos skill family (cos, acos-email-sort, and future acos skills) — who's a VIP/executive, who's staff, which vendors are trusted partners, which contacts should never be auto-declined or auto-filed, and how to sign off an email. Use when: setting up or updating an acos profile, add a VIP, add a direct report or staff member, add a partner vendor, add a protected contact, change my email signoff, who's on my VIP list, update acos aboutme, run acos aboutme."
version: "0.1"
...
2. Ask, in one short message, only for what's missing: the person's first name, full name, and how they'd like to sign off an email (e.g. "Trevor," "- Trevor," "Thanks, Trevor" — store just the name part; the surrounding phrasing is each template's own style); who their direct reports or core staff are (name and email each); who counts as an executive/VIP for priority handling (name and email each); which vendor companies are trusted partners rather than random vendors (name and domain each); and any specific individual contacts that should never be auto-declined or auto-filed even if a message from them looks like a form pitch (name, email, and why).
...
- This file holds identity and org-chart data only: names, emails, roles, and short reasons. Never store anything beyond that here — no financial, health, credential, or other Restricted-category data, even if someone tries to paste it in during enrollment. If that happens, leave it out and say why, per IT15.
