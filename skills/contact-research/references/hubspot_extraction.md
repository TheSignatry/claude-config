...
8. Write each contact's blocks with `merge_state.py --stage hubspot|associations|activity`. The `associations` fragment must carry **all** of these keys, as empty lists when nothing was found — `batch.py next` refuses to hand a contact to research while any of `contacts`, `surname_matches`, or `companies` is missing, and `derive.py` flags it:
   ...
   `household_members` may be omitted when no `Family`-type company was found; `spouse_in_hubspot` and `household_pair_candidate` are computed by `derive.py` — do not hand-write them.

## Redaction screen (both paths)

Before an engagement body is written to state, `derive.py`/`hubspot_pull.py` apply the redaction helper in `scripts/cr_common.py`: SSN pattern, card pattern, 9+ digit runs near "account/routing/acct", credential patterns (`password: …`, `username: …`), and a health/hardship lexicon matched at word starts. Research fragments get the pattern screen only — the lexicon is for notes about a person, not for prose about a company's sector or a street name. A hit replaces the whole body with `[redacted – possible Restricted content]`, increments `activity.redactions`, and adds a flag. Remind the user to report hits to the Technology Team (IT14 Policy 10) so the source record can be cleaned.
