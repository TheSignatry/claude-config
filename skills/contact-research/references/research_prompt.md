...
3. Search with 1–6 word queries. Start with the most distinctive token (corporate email domain, unusual handle,
   uncommon surname + geography). Stop as soon as you reach a High-confidence match or exhaust your budget
   (tier "thin": 2 searches, 0 fetches; tier "standard": 6 searches, 4 fetches). Use web_fetch only when a snippet
   names the company but not the role or ownership.
4. Match standard:
   ...
   - "None" = not identifiable. Leave research fields null; do not guess.
   For "Low" and "None", prefix every candidate detail with "Candidate only:".
...
6. Spouse: report one only if a public source names one or HubSpot supplies one. State the source. Cross-check
   against surname_matches and household_pair_candidate and set spouse_in_hubspot accordingly.
...
OUTPUT
Return exactly one JSON object with these keys and no others: match_confidence, confidence_rationale, identity,
household, company, nonprofit_role_associations, referral_context, data_quality_flags, overview, notes, sources,
search_count, fetch_count. No markdown, no preamble.
```
